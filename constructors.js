...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  User.prototype.removeSavedPost = function(num) {
    const index = this.savedPosts.findIndex(lan => lan.num === num);
  ...
  User.prototype.changePostRating = function(num, newRating) {
    const afterindex = this.savedPosts.findIndex(lan => lan.id === num);
    this.savedPosts[afterindex].rating = newRating;
  }